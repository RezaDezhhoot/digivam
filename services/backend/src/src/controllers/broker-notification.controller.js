...
const brokerWhere = (brokerId) => ({
  modelType: Notification.MODEL_TYPES.BROKER,
  modelId: Number(brokerId)
});

export const getBrokerNotificationSummary = async (req, res, next) => {
  try {
    const where = brokerWhere(req.auth.sub);
...
export const listBrokerNotifications = async (req, res, next) => {
  try {
    ...
    const [payload, summary] = await Promise.all([
      ...
      createNotificationSummary(brokerWhere(req.auth.sub))
    ]);
...
export const markBrokerNotificationRead = async (req, res, next) => {
  try {
...
export const markAllBrokerNotificationsRead = async (req, res, next) => {
  try {

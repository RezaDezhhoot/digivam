        ...
        <div v-if="activeStageTab === 'verify_broker' && selectedItem.canBrokerReview" class="section-card mt-3 review-card">
          ...
          <div class="payment-review-grid">
            <article v-for="item in paymentMethodCatalog" :key="item.value" class="payment-method-card" :class="{ active: reviewPaymentForm[item.value].enabled }">
              ...
              <input
                :value="formatAmountInputDisplay(reviewPaymentForm[item.value].amount)"
                class="form-control"
                type="text"
                inputmode="numeric"
                dir="ltr"
                ...
                @input="reviewPaymentForm[item.value].amount = extractRawAmountInput($event.target.value)"
              />
              <small v-if="Number(reviewPaymentForm[item.value].amount)" class="text-muted d-block mt-1">{{ formatAmountInWords(reviewPaymentForm[item.value].amount) }}</small>
